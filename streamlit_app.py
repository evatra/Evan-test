...
# Konfigurasi halaman
st.set_page_config(
    page_title="Dashboard-Predictive Maintenance PLTP Kamojang",
    page_icon="📈",
    layout="wide",
...
# Bagian sidebar
with st.sidebar:
    st.title('📈 Dashboard-Predictive Maintenance PLTP Kamojang')

    # Opsi untuk mengunggah file CSV
    uploaded_file = st.file_uploader("Upload a CSV file", type=["csv"])

    # Model selection
    selected_model = 'stokUsD4.h5'  # Using single LSTM model
...
def prediction(uploaded_file, selected_model, n_day, sample):
    ...
    try:
        # Add custom object scope for LSTM layer
        with tf.keras.utils.custom_object_scope({'LSTM': LSTM}):
            model = load_model(selected_model, compile=False, custom_objects={'LSTM': LSTM})
...
with st.form("maintenance_form"):
    ...
    maintenance_type = st.selectbox("Maintenance Type", 
        ["Predictive Maintenance", "Corrective Maintenance", "Breakdown Maintenance"])
...
with h5py.File('stokUsD4.h5', 'r') as f:
